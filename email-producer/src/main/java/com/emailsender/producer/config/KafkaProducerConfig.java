...
public class KafkaProducerConfig {
    ...
    public NewTopic emailRequestsTopic() {
        return TopicBuilder.name("email-requests").partitions(3).replicas(1).build();
    }

    @Bean
    public NewTopic emailRetryTopic() {
        return TopicBuilder.name("email-retry").partitions(3).replicas(1).build();
    }

    @Bean
    public NewTopic emailDlqTopic() {
        return TopicBuilder.name("email-dlq").partitions(3).replicas(1).build();
    }
}
